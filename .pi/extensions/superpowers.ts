...
The \`task\` tool has NO \`model:\` field — a template's \`model:\` line is inert; the agent TYPE carries the model, thinking level, and tools. For subagent-driven-development use this plugin's own agents: \`sdd-implementer\` (implementation, fix rounds 1-2), \`sdd-escalation-implementer\` (fix round 3, most capable tier), \`sdd-reviewer\` (task review, read-only, no shell), \`sdd-rereviewer\` (scoped re-review, cheap tier), \`sdd-final-reviewer\` (whole-branch review, most capable tier). The bundled \`task\` agent resolves to \`modelRoles.task\`, whatever that is — never use it for an SDD seat.
